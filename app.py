...
if user_question := st.chat_input("Ask Aryan AI any question from your school syllabus..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Aryan AI is analyzing..."):
            try:
                # If there is an uploaded textbook image, send both text and image data together
                if uploaded_file is not None:
                    response = st.session_state.chat_session.send_message([user_question, uploaded_file])
                else:
